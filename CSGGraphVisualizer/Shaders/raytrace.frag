...
////    r0 = min(r0,sphere(p - vec3(10.,10.,10.), 1.)); //union 13
//    //float light = sphere(p - getLightPos()-vec3(0,0,9), 0.4);
//    float light0 = sphere(p - getLightPos(), lights[0].radius);
//	r0 = min(r0,light0); //union with light
//    float light1 = sphere(p - lights[1].position, lights[1].radius);
//	r0 = min(r0,light1); //union with light
//    float light2 = sphere(p - lights[2].position, lights[2].radius);
//	r0 = min(r0,light2); //union with light
//	return r0;
//}
//
//Material material(vec3 p) {
...
////    Value light = Value(sphere(p - getLightPos()-vec3(0,0,9), 0.4),1);
//    Value light0 = Value(sphere(p - getLightPos(), lights[0].radius),4);
//	if(light0.d < r0.d)r0 = light0; //union with light
//    Value light1 = Value(sphere(p - lights[1].position, lights[1].radius),5);
//	if(light1.d < r0.d)r0 = light1; //union with light
//    Value light2 = Value(sphere(p - lights[2].position, lights[2].radius),6);
//	if(light2.d < r0.d)r0 = light2; //union with light
//    return colors[r0.mat];
//}
...
// Signed distance function for all of the spheres (union of spheres)
//float sdf(in vec3 p)
//{
//    float d = p.y+1.0;
...
//    for(int i=1; i<6; ++i)
//    {
//        float t = float(i)/float(5)*2.*pi + iTime*float(i)/2.;
//        d = min(d,
//           //sdSphere(p-vec3(cos(t),sin(t)+1.0,2.f)*2.,1.f)
//           sdSphere(p-vec3(cos(t)-1.0,sin(t*1.5) + 1.5,sin(t) + 1.f)*2.,1.f)
//           );
//    }    
//    return d;
//}
